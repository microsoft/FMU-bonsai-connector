...
def simulate_ssp(ssp_filename, start_time=0.0, stop_time=None, step_size=None, parameter_set=None, input={}):
    ...
    while time < stop_time:
        ...
        # *****
        # Set U input on the emachine_model system. (Note that the main system of this SSP doesn't have an input connector.)
        # *****
        for connector in connectors:
            input_names = ["U"]
            if connector.kind == 'input' and connector.name in input_names and connector.parent.name == 'emachine_model':
                connector.value = 10.0
        ...
        # *****
        for connector in connectors:
            output_names = ["M_mot", "wB", "U", "I"]
            if connector.kind == 'output' and connector.name in output_names:
                print(f"{time} {connector.parent.name} {connector.name} = {connector.value}")
